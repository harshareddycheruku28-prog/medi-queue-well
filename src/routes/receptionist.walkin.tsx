...
function Page() {
  ...
  const { data: doctors = [] } = useQuery<any[]>({
    queryKey: ["docs-walkin", departmentId],
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase
      .from("appointments")
      .insert({
        patient_id: receptionistId,
        doctor_id: doctorId,
        department_id: departmentId,
        appointment_date: today,
        slot_time: slot || "09:00:00",
          ...
          <CardContent className="text-center">
            <div className="rounded-lg border border-primary/30 bg-primary/5 p-4">
              ...
              <div className="mt-1 text-sm">
                {done.departments?.name} · {String(done.slot_time).slice(0, 5)}
              </div>
            </div>
            <Button
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Generates a token immediately.</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>Patient details</CardTitle>
          <CardDescription>Fill in the basics and assign a doctor.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={submit} className="grid gap-3 sm:grid-cols-2">
            ...
            <div className="space-y-1">
              ...
              <Input
                type="time"
                value={slot.slice(0, 5)}
                onChange={(e) => setSlot(e.target.value + ":00")}
              />
            </div>
            <div className="space-y-1 sm:col-span-2">
              <Label>Symptoms</Label>
              <Textarea value={symptoms} onChange={(e) => setSymp(e.target.value)} rows={3} />
            </div>
