...
function Page() {
  ...
  const { data: appts = [], refetch } = useQuery({
    ...
    queryFn: async () => {
      let query = supabase
        .from("appointments")
        .select(
          "*, departments(name, code), doctors(profiles:profile_id(full_name)), patient:profiles!appointments_patient_id_fkey(full_name, phone, email)",
        )
  ...
  const cancel = async (id: string) => {
    ...
    if (error) toast.error(error.message);
      ...
      qc.invalidateQueries({ queryKey: ["search-appts"] });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Search Appointments</h1>
        <p className="text-muted-foreground">By token code or date.</p>
      </div>
      <Card>
        <CardContent className="grid gap-3 p-4 sm:grid-cols-[1fr_180px_auto]">
          <div className="space-y-1">
            <Label>Token code</Label>
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="e.g. GEN-001" />
          </div>
          <div className="space-y-1">
            <Label>Date</Label>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Results ({appts.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {appts.length === 0 ? (
                ...
                >
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {a.appointment_date} · {a.slot_time?.slice(0, 5)} · {a.departments?.name} ·
                      Dr. {a.doctors?.profiles?.full_name}
                    </div>
                  </div>
                  {a.status === "waiting" && (
