...
export function LiveQueueBoard({ highlightToken }: { highlightToken?: string }) {
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {data.depts.map((d) => {
        const dAppts = data.appts.filter(
          (a) => a.department_id === d.id && a.status !== "cancelled" && a.status !== "completed",
        );
        const current = data.appts.find(
          (a) => a.department_id === d.id && a.status === "in_progress",
        );
